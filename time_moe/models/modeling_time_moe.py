...
def load_balancing_loss_func(
    ...
    # situation 2:zhu
        ...
        # 2. expand shape of attn_mask to [num_hidden_layers, batch_size, sequence_length, 2, num_experts]
        # 3. flatten first 2 dim, reshape to [num_layers * batch_size * seq_len, 2, num_experts], same shape of expert_mask
...
class TimeMoeInputEmbedding(nn.Module):
    ...
    Embed [batch_sz, seq_len, input_sz] to [batch_sz, seq_len, hidden_sz]
    """
...
class TimeMoeMLP(TimeMoeTemporalBlock):
    """
    Wrapper of class TimeMoeTemporalBlock
    """
    def __init__(self, hidden_size: int, intermediate_size: int, hidden_act: str):
...
class TimeMoeAttention(nn.Module):
    ...
    ie implementation of Causal Attention layer,
    but where is causal_attn_mask?
    """
    ...
    def forward(
        ...
        # reshape attn_output: [bsz, num_heads, q_len, head_dim] → [bsz, q_len, hidden_size]
...
# Mind is_causal
# Mind that diff ver flash_attn package handle causal_attn_mask differently
# flash_attn may not support custom attn mask, see comment in TimeMoeFlashAttention2._flash_attention_forward()
class TimeMoeFlashAttention2(TimeMoeAttention):
    """
    Optimized TimeMoeAttention by flash attention 2 algo
    Mind that diff ver flash_attn package handle causal_attn_mask differently
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        # check flash_attn package version, different ver correspond to different causal_attn_mask
        # if 
    ...
    def forward(
        ...
        """
        Calculate attention with flash_attn, keep linear project the same
        """

        # KV cache supportness check
        ...
        # flash attention not cal or return output_attentions
        output_attentions = False

        bsz, q_len, _ = hidden_states.size()    # q_len for query_len
        ...
        # 
        # query: [bsz, q_len, num_heads * head_dim] → [bsz, num_heads, q_len, head_dim]
        # kv: [bsz, q_len, num_kv_heads * head_dim] → [bsz, num_kv_heads, q_len, head_dim]
        query_states = query_states.view(bsz, q_len, self.num_heads, self.head_dim).transpose(1, 2)
        key_states = key_states.view(bsz, q_len, self.num_key_value_heads, self.head_dim).transpose(1, 2)
        value_states = value_states.view(bsz, q_len, self.num_key_value_heads, self.head_dim).transpose(1, 2)
        ...
        # only dropout when training not inferencing, why dont do it in TimeMoeAttention?
        ...
        # reshape (concat multi attention head output) & forward through o_proj
    ...
    def _flash_attention_forward(
        ...
        # Mind this
        # Use self.is_causal if package flash_attn>=2.10, 
        # otherwise use it when query_length != 1
        if not self._flash_attn_uses_top_left_mask:
            causal = self.is_causal
        else:
            # TODO: Remove the `query_length != 1` check once Flash Attention for RoCm is bumped to 2.1. For details, please see the comment in LlamaFlashAttention2 __init__.
            # may need to check the latest ver repo, it seems that it will be soon remove
        ...
        # call flash_attn
        # Mind this: flash_attn dont use input attn_mask ↑
...
# Mind this: 或许可以自定义Attention-block?
...
class TimeMoeDecoderLayer(nn.Module):
    """
    Implement of TimeMoE transformer block
    """
    def __init__(self, config: TimeMoeConfig, layer_idx: int):
        ...
        # Before- & After-Attention Norm layer
    ...
    def forward(
        ...
        # before-attention norm
        hidden_states = self.input_layernorm(hidden_states)

        # Self Attention
        # Mind this: where is casual attn?? it is because this is decoder? --TimeMoE is decoder-only
...
class TimeMoePreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, torch.nn.Linear):
            ...
            module.weight.data.normal_(mean=0.0, std=std)
            if module.padding_idx is not None:  # why?
...
class TimeMoeModel(TimeMoePreTrainedModel):
    ...
    Embedding layer + TimeMoE backbone: TimeMoeDecoderLayer * N
    ...
    def forward(
        ...
        # Mind this: why [batch_size, seq_len, input_size] ← input_size? [batch_sz, seq_len, features]
        # set default param
        ...
        # gradient checkpoint vs KV cache
        ...
        # maybe we could use sliding_window to generate patch-wise causal_attn_mask?
        attention_mask = _prepare_4d_causal_attention_mask(
            attention_mask,
            (batch_size, seq_length),
            inputs_embeds,  # Mind this: it use inputs_embeds rather then input_ids → only use it for embedding.dtype & device
        ...
        return MoeModelOutputWithPast(
            ...
            past_key_values=next_cache,         # updated KV cache, use for auto-regression
...
class TimeMoeOutputLayer(nn.Module):
    """
    Prediction head: a MLP from [hidden_size] → [input_size * horizon_length], mind that input_size == num_feature
    """
...
class TimeMoeForPrediction(TimeMoePreTrainedModel, TSGenerationMixin):
    ...
    def forward(
        ...
        # ↑ why? the return of TimeMoeModel.forward is (last_hidden_state, past_key_values, hidden_states, attentions, router_logits)
        # where's router_logits
        outputs = self.model(
            input_ids=input_ids,
            attention_mask=attention_mask,
            position_ids=position_ids,
        ...
        loss = None
        aux_loss = None
        if labels is not None:  # when training
            # AutoRegressive loss
            ar_loss = 0.0
    ...
    def calc_ar_loss(self, predictions, labels, loss_masks, horizon_length):
        ...
        # reshape labels to [batch_sz, seq_len, input_sz] 
        ...
        # multi-step prediciton
        if horizon_length > 1:

            # reshape predictions: [batch_sz, seq_len, input_sz * horizon_len] → [batch_sz, seq_len, ]
            ...
            # shape: [B, input_size, seq_len] -> [B, input_size, seq_len + horizon_length -1]
            ...
            # shape -> [B, seq_len, horizon_length, input_size]
