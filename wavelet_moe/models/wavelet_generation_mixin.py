...
class WaveletGenerationMixin(GenerationMixin):
    ...
    def _greedy_search(
        ...
        if "inputs_embeds" in model_kwargs:
            token_num = model_kwargs["inputs_embeds"].shape[1]
        this_peer_finished = False
        unfinished_sequences = torch.ones(batch_size, dtype=torch.long, device=input_ids.device)
        model_kwargs["cache_position"] = torch.arange(token_num, device=input_ids.device)
    ...
    def _update_model_kwargs_for_generation(
        ...
        """
        Call after every forward during inferecing.
        """
